...
Now that we have fitted our model and identified any influential points, dealing with them however we decided to, we can take a look at the summary of our model and begin performing our analysis. The important things we will want to note here is a residual analysis on our now completed model, taking a look at the summary statistics and interpreting values such as our adjusted R^2 to see how well we fit the model, the residual standard error to assess the quality of our fit, and of course conduct a hypothesis test to see if there is a significant linear relationship between at least one of the predictors and the response. Our hypothesis test would consider:
	Ho : β1 = β2 = . . . = βk = 0 vs.
